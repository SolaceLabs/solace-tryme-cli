...
#
# Usage: ./test_reply_basic_parameters.sh [START_TEST_NUMBER]
# Example: ./test_reply_basic_parameters.sh 10  # Start from test 10

# Test script for reply command basic parameters
#
# Usage: ./test_reply_basic_parameters.sh [START_TEST_NUMBER]
# Example: ./test_reply_basic_parameters.sh 10  # Start from test 10
...
echo -e "${CYAN}=== Reply Command Basic Parameters Test ===${NC}\n"
...
run_test_if_enabled 1 "Basic help command (short form)" "node $CLI_PATH reply $LINT_OPTION -h" 0

# Test 2: Basic help output (long form)
run_test_if_enabled 2 "Basic help command (long form)" "node $CLI_PATH reply $LINT_OPTION --help" 0

# Test 3: Basic parameters validation (short forms)
run_test_if_enabled 3 "Basic parameters (short forms)" "node $CLI_PATH reply $LINT_OPTION -t 'test/topic' -m 'test message' --url ws://localhost:8008 --vpn default --username default --password default --exit-after 1" 0

# Test 4: Basic parameters validation (long forms)
run_test_if_enabled 4 "Basic parameters (long forms)" "node $CLI_PATH reply $LINT_OPTION --topic 'test/topic' --message 'test message' --url ws://localhost:8008 --vpn default --username default --password default --exit-after 1" 0

# Test 5: Default message parameter
run_test_if_enabled 5 "Default message parameter" "node $CLI_PATH reply $LINT_OPTION --topic 'test/topic' --default-message --url ws://localhost:8008 --vpn default --username default --password default --exit-after 1" 0

# Test 6: Empty message parameter
run_test_if_enabled 6 "Empty message parameter" "node $CLI_PATH reply $LINT_OPTION --topic 'test/topic' --empty-message --url ws://localhost:8008 --vpn default --username default --password default --exit-after 1" 0

# Test 7: File message parameter
if [ "${STM_TEST_EXECUTION:-0}" = "1" ]; then
    run_test_if_enabled 7 "File message parameter" "node $CLI_PATH reply $LINT_OPTION --topic 'test/topic' --file 'nonexistent.txt' --url ws://localhost:8008 --vpn default --username default --password default --exit-after 1" 1
else
    run_test_if_enabled 7 "File message parameter" "node $CLI_PATH reply $LINT_OPTION --topic 'test/topic' --file 'nonexistent.txt' --url ws://localhost:8008 --vpn default --username default --password default --exit-after 1" 0
fi

# Test 8: Stdin message parameter
run_test_if_enabled 8 "Stdin message parameter" "timeout 5 echo 'test input' | node $CLI_PATH reply $LINT_OPTION --topic 'test/topic' --stdin --url ws://localhost:8008 --vpn default --username default --password default --exit-after 1" 0

# Test 9: Output mode parameter
run_test_if_enabled 9 "Output mode parameter" "node $CLI_PATH reply $LINT_OPTION --topic 'test/topic' --message 'test' --output-mode FULL --url ws://localhost:8008 --vpn default --username default --password default --exit-after 1" 0

# Test 10: Invalid parameter combination (conflicting message options)
run_test_if_enabled 10 "Invalid parameter combination (conflicting message options)" "node $CLI_PATH reply $LINT_OPTION --topic 'test/topic' --message 'test' --default-message --url ws://localhost:8008 --vpn default --username default --password default" 1

# Test 11: Missing required topic parameter (but falls back to default topic)
run_test_if_enabled 11 "Missing required topic parameter" "node $CLI_PATH reply $LINT_OPTION --message 'test' --url ws://localhost:8008 --vpn default --username default --password default --exit-after 1" 0

# Test 12: Invalid output mode parameter
run_test_if_enabled 12 "Invalid output mode parameter" "node $CLI_PATH reply $LINT_OPTION --topic 'test/topic' --message 'test' --output-mode INVALID --url ws://localhost:8008 --vpn default --username default --password default --exit-after 1" 1
