...
#
# Usage: ./test_feed_run_basic_parameters.sh [START_TEST_NUMBER]
# Example: ./test_feed_run_basic_parameters.sh 10  # Start from test 10

# Test script for feed run command basic parameters
#
# Usage: ./test_feed_run_basic_parameters.sh [START_TEST_NUMBER]
# Example: ./test_feed_run_basic_parameters.sh 10  # Start from test 10
...
echo -e "${CYAN}=== Feed Run Command Basic Parameters Test ===${NC}\n"
...
run_test_if_enabled 1 "Basic help command (short form)" "node $CLI_PATH feed run $LINT_OPTION -h" 0
...
# Test 14: Use defaults parameter
run_test_if_enabled 14 "Use defaults parameter" "node $CLI_PATH feed run $LINT_OPTION --feed-name 'Core Banking' --use-defaults --url ws://localhost:8008 --vpn default --username default --password default" 0

# Test 16: Invalid count parameter
run_test_if_enabled 16 "Invalid count parameter" "node $CLI_PATH feed run $LINT_OPTION --feed-name 'Core Banking' --count -1 --url ws://localhost:8008 --vpn default --username default --password default" 1

# Test 17: Invalid interval parameter
run_test_if_enabled 17 "Invalid interval parameter" "node $CLI_PATH feed run $LINT_OPTION --feed-name 'Core Banking' --count 1 --interval -1000 --url ws://localhost:8008 --vpn default --username default --password default" 1

# Test 18: Invalid output mode parameter
run_test_if_enabled 18 "Invalid output mode parameter" "node $CLI_PATH feed run $LINT_OPTION --feed-name 'Core Banking' --count 1 --output-mode INVALID --url ws://localhost:8008 --vpn default --username default --password default" 1

# Test 19: Invalid initial delay parameter
run_test_if_enabled 19 "Invalid initial delay parameter" "node $CLI_PATH feed run $LINT_OPTION --feed-name 'Core Banking' --count 1 --initial-delay -500 --url ws://localhost:8008 --vpn default --username default --password default" 1

# Test 20: Non-existent feed name
if [ "${STM_TEST_EXECUTION:-0}" = "1" ]; then
    run_test_if_enabled 20 "Non-existent feed name" "node $CLI_PATH feed run $LINT_OPTION --feed-name 'non-existent-feed' --count 1 --url ws://localhost:8008 --vpn default --username default --password default" 1
else
    run_test_if_enabled 20 "Non-existent feed name" "node $CLI_PATH feed run $LINT_OPTION --feed-name 'non-existent-feed' --count 1 --url ws://localhost:8008 --vpn default --username default --password default" 0
fi
