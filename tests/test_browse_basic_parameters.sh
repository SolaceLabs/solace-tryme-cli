...
# Test script for browse command basic parameters
#
# Usage: ./test_browse_basic_parameters.sh [START_TEST_NUMBER]
# Example: ./test_browse_basic_parameters.sh 10  # Start from test 10
...
echo -e "${CYAN}=== Browse Command Basic Parameters Test ===${NC}\n"
...
run_test_if_enabled 1 "Basic help command (short form)" "node $CLI_PATH browse $LINT_OPTION -h" 0

# Test 2: Basic help output (long form)
run_test_if_enabled 2 "Basic help command (long form)" "node $CLI_PATH browse $LINT_OPTION --help" 0

# Test 3: Basic parameters validation (short forms)
run_test_if_enabled 3 "Basic parameters (short forms)" "node $CLI_PATH browse $LINT_OPTION -q 'test-queue-$TIMESTAMP' --exit-after 1 --url ws://localhost:8008 --vpn default --username default --password default" 0

# Test 4: Basic parameters validation (long forms)
run_test_if_enabled 4 "Basic parameters (long forms)" "node $CLI_PATH browse $LINT_OPTION --queue 'test-queue-$TIMESTAMP' --exit-after 1 --url ws://localhost:8008 --vpn default --username default --password default" 0

# Test 5: Output mode parameter
run_test_if_enabled 5 "Output mode parameter (DEFAULT)" "node $CLI_PATH browse $LINT_OPTION --queue 'test-queue-$TIMESTAMP' --output-mode DEFAULT --exit-after 1 --url ws://localhost:8008 --vpn default --username default --password default" 0

run_test_if_enabled 5 "Output mode parameter (PROPS)" "node $CLI_PATH browse $LINT_OPTION --queue 'test-queue-$TIMESTAMP' --output-mode PROPS --exit-after 1 --url ws://localhost:8008 --vpn default --username default --password default" 0

run_test_if_enabled 5 "Output mode parameter (FULL)" "node $CLI_PATH browse $LINT_OPTION --queue 'test-queue-$TIMESTAMP' --output-mode FULL --exit-after 1 --url ws://localhost:8008 --vpn default --username default --password default" 0

# Test 6: Exit after parameter
run_test_if_enabled 6 "Exit after parameter" "node $CLI_PATH browse $LINT_OPTION --queue 'test-queue-$TIMESTAMP' --exit-after 5 --url ws://localhost:8008 --vpn default --username default --password default" 0

# Test 7: Missing required parameter (no queue)
run_test_if_enabled 7 "Missing required parameter (no queue)" "node $CLI_PATH browse $LINT_OPTION --exit-after 1 --url ws://localhost:8008 --vpn default --username default --password default" 1

# Test 8: Topic subscription not supported for browse
run_test_if_enabled 8 "Topic option not supported for browse" "node $CLI_PATH browse $LINT_OPTION --queue 'test-queue-$TIMESTAMP' --topic 'test/topic' --exit-after 1 --url ws://localhost:8008 --vpn default --username default --password default" 1

# Test 9: Create-if-missing not supported for browse
run_test_if_enabled 9 "Create-if-missing option not supported for browse" "node $CLI_PATH browse $LINT_OPTION --queue 'test-queue-$TIMESTAMP' --create-if-missing true --exit-after 1 --url ws://localhost:8008 --vpn default --username default --password default" 1

# Test 10: Invalid output mode parameter
run_test_if_enabled 10 "Invalid output mode parameter" "node $CLI_PATH browse $LINT_OPTION --queue 'test-queue-$TIMESTAMP' --output-mode INVALID --exit-after 1 --url ws://localhost:8008 --vpn default --username default --password default" 1

# Test 11: Invalid exit after parameter
run_test_if_enabled 11 "Invalid exit after parameter" "node $CLI_PATH browse $LINT_OPTION --queue 'test-queue-$TIMESTAMP' --exit-after -1 --url ws://localhost:8008 --vpn default --username default --password default" 1
