...
// Logger.debug(localStorage.getItem('myFirstKey'));
...
function onConnectionLost(responseObject) {
  if (responseObject.errorCode !== 0)
	Logger.debug("onConnectionLost:"+responseObject.errorMessage);
};
function onMessageArrived(message) {
  Logger.debug("onMessageArrived:"+message.payloadString);
  client.disconnect();
};
 * </pre></code>
 * @namespace Paho.MQTT
        ...
        if (this.connected) {
          ...
          if (this.onConnectionLost) {
            this.onConnectionLost({
              errorCode, errorMessage: errorText, reconnect: this.connectOptions.reconnect, uri: this._wsuri
            });
          }
      ...
      /**
		 ...
		 */
		 this.publish = function (topic, payload, qos, retained) {
			 Logger.debug('Publising message to: ', topic);
			 let message;
